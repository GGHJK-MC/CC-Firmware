...
local function loadLocalVersion()
    if not fs.exists(VER_PATH) then return "0" end
    local f = fs.open(VER_PATH, "r")
...
local function saveLocalVersion(v)
    ...
    local f = fs.open(VER_PATH, "w")
    if f then f.write(v); f.close() end
end

local localVersion = loadLocalVersion()
...
if remoteVersion and isNewerVersion(remoteVersion, localVersion) then
    ...
    if fs.exists("/fw") then
        for _, file in ipairs(fs.list("/fw")) do
            if file ~= "version.txt" and file ~= "seccfg" then
...
if isUpdate or not integrityOk then
    ...
    saveLocalVersion(remoteVersion or localVersion)

    local frdFile = fs.open("/fw/seccfg/fwrd", "w")
    if frdFile then frdFile.write(fwrdVal); frdFile.close() end

    if not integrityOk and not isUpdate then
        os.pullEvent = nativePull

        local function verfail()
            ...
            if f then
                ...
                if ok and img and #img > 0 then
                    local iw = (type(img[1]) == "table") and #img[1] or 0
                    paintutils.drawImage(img, math.max(1, math.floor((tw - iw) / 2) + 1), 2)
                end
            end
            ...
            local function wc(y, text, fg)
                ...
                term.setCursorPos(math.floor((tw - #text) / 2) + 1, y)
                term.write(text)
            end
            ...
            wc(th, "GGHJK UEFI | Boot Halt - Integrity Error",           colors.gray)
